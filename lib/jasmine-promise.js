                ...
                waitsForExpectation(function(){
                  this.expectations = _.reject(this.expectations, function( expect ){
                    return _.reduce(this, function(val, key){ 
                      return _.isEqual(key[0], this) ? true : val  //TODO: this only matches strings and not objects - puts doesn't match
    ...
    jasmine.addToSpec({
      ...
      gets: function (args) {
         ...
         $.mockjax( {
           ...
           type: 'GET',
           status: status || OK,
           headers: _.extend({ Accept: accept || Json}, headers), // see http://tech.groups.yahoo.com/group/rest-discuss/message/14432
      ...
      puts: function (args) {
        ...
        this.expectation("PUT", url, status /*, content*/) // TODO: expectations doesn't match objects
      },
      deletes: function(args){
        // TODO: cleanup argument processing when needed
         var url = args[0],
           status = args[1][0]
         ...
         $.mockjax( {
           ...
           type: 'DELETE',
           responseText: '',
           status: status || OkNoEntity,
           async: false
         })           

         this.expectation("DELETE", url, status)
      },
